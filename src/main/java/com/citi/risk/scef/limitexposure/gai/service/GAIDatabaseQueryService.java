...
/**
 * Executes GAI SQL files using the ADMCEF ODS DataSource from ScefOdsDBModule.
 *
 * ScefOdsDBModule exposes DataSource (after adding expose(DataSource.class)).
 * This gives us:
 *   - CoreMybatisDataSourceProvider with env-prefix resolution
 *   - DBCP connection pooling (scef-ods.DBCP.initialSize=8, maxTotal=30)
 *   - CyberArk password integration
 *   - Same DB as all other SCEF requests
 *
 * Requires ONE change to ScefOdsDBModule.getModule() — see SCEF_ODS_MODULE_CHANGES.txt
 */
public class GAIDatabaseQueryService {
    ...
    @Inject
    public GAIDatabaseQueryService(@Named("gai-ods") DataSource dataSource) {
        this.jdbc = new NamedParameterJdbcTemplate(dataSource);
        logger.info("[GAI] GAIDatabaseQueryService initialised with ADMCEF ODS DataSource");
    }
